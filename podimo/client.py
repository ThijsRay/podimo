...
from podimo.utils import async_wrap
from podimo.utils import generateHeaders as gHdrs
from podimo.utils import is_correct_email_address, randomFlyerId, token_key
...
class PodimoClient:
    ...
    async def post(self, headers, query, variables, scraper):
        ...
        if response is None:
            raise RuntimeError(
                f"Could not receive response for query: {query.strip()[:30]}..."
            )
        if response.status_code != 200:
            raise RuntimeError(
                f'Podimo returned an error code. Response code was: {response.status_code} for query "{query.strip()[:30]}..."'
            )
        result = response.json()["data"]
        if result is None:
            raise RuntimeError(
                f"Podimo returned no valid data for query {query.strip()[:30]}"
            )
        return result
    ...
    async def getPreregisterToken(self, scraper):
        ...
        if not self.preauth_token:
            raise RuntimeError(
                "Podimo did not provide a tokenWithPreregisterUser token"
            )
    ...
    async def podimoLogin(self, scraper):
        ...
        if not tokenWithCredentials:
            raise ValueError(
                "Invalid Podimo credentials, did not receive tokenWithCredentials"
            )
    ...
    async def getPodcasts(self, podcast_id, scraper):
        ...
        while True:
            ...
            if offset == 0:
                ...
                logging.debug(
                    f"Fetched podcast '{podcastName}' ({podcast_id}) directly"
                )
                fullResult = result
            else:
